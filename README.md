    ...
    steps:
      ...
      - uses: pgup-ai/jbot-review-action@v0 # latest v0.x.y
        with:
          ...
          # auxiliary sessions on a cheaper provider than the main model.
          review-passes: '1'
...
| ---------------------------- | -------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`                   | No       | from `model`          | Deprecated — qualify `model` instead; pins the provider when set (`JBOT_REVIEW_PROVIDER`). Valid ids: opencode, opencode-go, deepseek, openai, openai-compatible, anthropic, google, openrouter, nvidia, zai-coding-plan, kimi-for-coding, xai, xiaomi-token-plan-sgp, fireworks-ai, poolside, devin, commandcode, cursor, qoder, codex, cline, cline-pass, grok, kilo |
| `model`                      | No       | `opencode` default    | `provider/model` reference, or a comma-separated pool naming one provider; required for `openai-compatible`                                                                                                                                                                                                                                                            |

    ...
    steps:
      ...
      - uses: pgup-ai/jbot-review-action@v0 # latest v0.x.y
        with:
          ...
          # findings adversarially verified before posting.
          review-passes: '1'
...
| `verify-findings`            | No       | `true`                | Re-check all findings before posting; uncertain findings stay in run diagnostics, not PR comments, except up to two unchecked P0–P2 findings a run posted as Unverified                                                                                                                                                                                                                                                                                         |
...
| `prompt-cache`               | No       | `true`                | Prompt caching for OpenCode-server sessions (`setCacheKey`); cuts input-token cost on models that honor it; models marked unsupported omit the cache key. The opt-in pi engine (`sdk-engine: auto`) caches provider-side on its own, so this only affects OpenCode-server sessions                                                                                    |
| `skip-doc-only`              | No       | `false`               | Set `true` to skip the review (no model call) when the PR changes only documentation/diagram assets (`.md`, `.svg`, `.drawio`, `.pdf`, …); the reaction is left unchanged (a docs push does not change the verdict). Off by default: review guidance lives in docs, so their changes are reviewed too.                                                                |
...
Use repository or organization Actions variables `JBOT_REVIEW_MODEL` and
...
one candidate, and a one-entry pool always does.
...
catalog does not advertise support for it. When `openai-compatible` is among the
pool's providers, pass its namespaced key and base URL.
...
Every provider the pool draws on needs its own key input or env var — a key is
never reused across providers.
...
The legacy `provider` input still works and keeps the previous resolution
exactly. Setting it pins the provider: an unprefixed id belongs to it, a
...
Model-key providers run on the OpenCode server by default. Set
`sdk-engine: auto` to route a session through the in-process
[pi](https://pi.dev/) SDK instead when its catalog contains the selected model;
the Kimi providers and `openai-compatible` always use OpenCode, while Poolside
uses its direct chat-completions backend.
